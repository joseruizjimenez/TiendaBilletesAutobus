			...
			<div id="page-bgbtm">
				<div id="content">
				  <div class="post" align="center">
                      <h2 class="title">Gestionar billetes adquirido</h2>
                      ...
                      <form method="post" action="/TiendaBilletesAutobus/go">
                            <input name="to" value="buscarBillete" type="hidden">
                            <%HashMap<UUID,Servicio> serviciosC = (HashMap<UUID,Servicio>) servicios;%>
                            <p>Inserte el localizador o el numero de tarjeta (el resto de campos son obligatorios):</p>
                            <p style="color: blue; text-align: right; padding-right: 240px;">
                                Num. Tarjeta <input name="numTarjeta" value="" type="text" /></br>
                                Localizador <input name="localizador" value="" type="text" /></br></br>
                                NIF <input name="dni" value="" type="text" /></br>
                                Buscar por <select name="fechaaaa"><option>Fecha de compra</option><option>Fecha de viaje</option></select></br>
                                Fecha  <select name="day"><%for(int i=1;i<=31;i++){%><option><%=i%></option><%}%></select>&nbsp;<select name="month">
                                    <%for(int i=1;i<=12;i++) {%><option><%=i%></option><%}%></select>&nbsp;<select name="year">
                                    <%for(int i=2012;i<2015;i++) {%><option><%=i%></option><%}%></select>
                            </p>
                            <p></p>
