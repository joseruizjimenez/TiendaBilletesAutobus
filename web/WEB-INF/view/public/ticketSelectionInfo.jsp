			...
			<div id="page-bgbtm">
				<div id="content">
				  <div class="post" align="center">
                      <h2 class="title">Confirme los datos</h2>
                      ...
                      <%for(BilleteVendido billete : billetesReservadosC) {%>
                          <div class="billete">
                              <p><b>Viajero</b>: <%=billete.getNombreViajero()%></p>
                              <p><b>NIF viajero</b>: <%=billete.getDniViajero()%></p>
                              <p><b>Origen</b>: <%=billete.getServicio().getRuta().getOrigen()%></p>
                              <p><b>Hora salida</b>: <%=billete.getServicio().getHoraSalida()%></p>
                              <p><b>Destino</b>: <%=billete.getServicio().getRuta().getDestino()%></p>
                              <p><b>Hora llegada</b>: <%=billete.getServicio().getHoraLlegada()%></p>
                              <p><b>Itinerario</b>: <%=billete.getServicio().getRuta().getParadasAsString()%></p>
                              <p><b>Plaza</b>: <%=billete.getNumAsiento()%> de <%=billete.getServicio().getTotalPlazas()%> asientos</p>
                              <p><b>Familia numerosa</b>: <%=billete.getOtraInfo()%></p>
                              <p><b>Precio</b>: <%=billete.getServicio().getPriceAsFormattedString()%></p>
                          </div>
                      <%}%>
                      <%if(billetesReservadosC.size() != 1) {%></div><%}%>
                      <p></p>
                      <p>
                          <a href="/TiendaBilletesAutobus/go?to=cancelado" style="color: red; font-weight: bold;" onclick="return confirm('¿Cancelar la compra?');">CANCELAR</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                          <%if(billeteGestion == null || billeteGestion.getServicio() == null) {%>
                              <a href="/TiendaBilletesAutobus/go?to=aceptado" style="color: #2EB902; font-weight: bold;" onclick="return confirm('¿Acepta las condiciones de venta?');">PROCEDER CON EL PAGO</a>
                          <%} else {%>
                              <a href="/TiendaBilletesAutobus/go?to=aplicarCambio" style="color: #2EB902; font-weight: bold;" onclick="return confirm('¿Desea proceder con el cambio?');">CAMBIAR BILLETE</a>
                          <%}%>
                      </p>
				  </div>
				</div>
