...
<body>
    ...
    <jsp:useBean id="idServicioIda" scope="request" class="java.lang.String" />
    <jsp:useBean id="idServicioVuelta" scope="request" class="java.lang.String" />
    <jsp:useBean id="numBilletes" scope="request" class="java.lang.String" />
			...
			<div id="page-bgbtm">
				<div id="content">
				  <div class="post" align="center">
                      <h2 class="title">Comprar Billetes</h2>
                      ...
                      <form method="post" action="/TiendaBilletesAutobus/go">
                            <input name="to" value="seleccionForm" type="hidden">
                            <%HashMap<UUID,Servicio> serviciosC = (HashMap<UUID,Servicio>) servicios;%>
                            <%for(int i=1;i <= Integer.parseInt(numBilletes);i++) {
                                if(idServicioVuelta != null && !"".equals(idServicioVuelta)) {%>
                                <b><%=i%>.</b> Inserte los datos del viajero para los servicios de ida y vuelta: <b><%=serviciosC.get(UUID.fromString(idServicioIda)).getRuta().getOrigen()%> - <%=serviciosC.get(UUID.fromString(idServicioIda)).getRuta().getDestino()%></b></br></br>
                                <%} else {%>
                                <b><%=i%>.</b> Inserte los datos del viajero para el servicio de ida: <b><%=serviciosC.get(UUID.fromString(idServicioIda)).getRuta().getOrigen()%> - <%=serviciosC.get(UUID.fromString(idServicioIda)).getRuta().getDestino()%></b></br></br>
                                <%}%>
                                <p style="color: blue; text-align: right; padding-right: 240px;">Nombre del viajero <input name="nombre<%=i%>" value="" type="text" /></br>
                                Apellidos del viajero <input name="apellidos<%=i%>" value="" type="text" /></br>
                                NIF del viajero <input name="dni<%=i%>" value="" type="text" /></p>
                            <%}%>
                            <p></p>
                            <button>Elegir asiento</button>
                      </form>
				  </div>
				</div>
				<!-- end #content -->
