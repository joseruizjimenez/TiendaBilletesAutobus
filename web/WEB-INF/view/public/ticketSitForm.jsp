...
<body>
    ...
    <jsp:useBean id="billetesReservados" scope="session" class="java.util.ArrayList" />
    <jsp:useBean id="asientosReservadosIda" scope="request" class="java.util.ArrayList" />
    <jsp:useBean id="asientosReservadosVuelta" scope="request" class="java.util.ArrayList" />
				...
				<div id="content">
				  <div class="post" align="center">
                      ...
                      <form method="post" action="/TiendaBilletesAutobus/go">
                            <input name="to" value="asientos" type="hidden">
                            <%HashMap<UUID,Servicio> serviciosC = (HashMap<UUID,Servicio>) servicios;
                            ArrayList<BilleteVendido> billetesReservadosC = (ArrayList<BilleteVendido>) billetesReservados;
                            ArrayList<Integer> asientosReservadosIdaC = (ArrayList<Integer>) asientosReservadosIda;
                            ArrayList<Integer> asientosReservadosVueltaC = (ArrayList<Integer>) asientosReservadosVuelta;%>
                            <%if(billetesReservadosC.size() != 1) {%><div id="billetes"><%}%>
                            <%int j = 1;
                            for(BilleteVendido billete : billetesReservadosC) {%>
                                <div class="billete">
                                <b><%=j%>.</b> Seleccione el asiento para: <b><%=billete.getNombreViajero()%></b> (<%=billete.getServicio().getRuta().getOrigen()+" - "+billete.getServicio().getRuta().getDestino()%>)</br>
                                <div class="asientos">
                                    <%int col = 1;
                                    for(int plaza=1; plaza<=billete.getServicio().getTotalPlazas(); plaza++) {
                                        if(col == 3) {%>
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                        <%} else if(col == 5) {%>
                                            </br>
                                        ...
                                        if(billetesReservadosC.get(0).getServicio().getRuta().getOrigen().equals(billete.getServicio().getRuta().getOrigen())) {
                                            if(asientosReservadosIdaC.contains(plaza)) {%>
                                                <input type="radio" name="numPlaza<%=j%>" value="<%=plaza%>" disabled="disabled">
                                            <%} else {%>
                                                <input type="radio" name="numPlaza<%=j%>" value="<%=plaza%>">
                                            <%}
                                        } else {
                                            if(asientosReservadosVueltaC.contains(plaza)) {%>
                                                <input type="radio" name="numPlaza<%=j%>" value="<%=plaza%>" disabled="disabled">
                                            <%} else {%>
                                                <input type="radio" name="numPlaza<%=j%>" value="<%=plaza%>">
                                            <%}
                                ...
                                <p style="color: blue;">Familia numerosa: <select name="familia<%=j%>"><option>no</option><option>general</option><option>especial</option></select></br>
                                Equipaje especial (mascota, bici...) <input name="otros<%=j%>" value="" type="text" /></p>
                                </div>
                                <%j++;
                            }%>
                            <%if(billetesReservadosC.size() != 1) {%></div><%}%>
                            <p></p>
                            <button>Reservar y continuar</button>
                      </form>
				  </div>
				</div>
				<!-- end #content -->
